# python ./7_ocr_card.py -i images/card.png
...
ap.add_argument("-r", "--reference", type=str, default="images/reference.png", help="path to reference OCR-A image")
...
# ref = cv2.threshold(ref, 10, 255, cv2.THRESH_BINARY_INV)[1] # black over white
ref = cv2.GaussianBlur(ref, (3, 3), 3)
ref = cv2.threshold(ref, 10, 255, cv2.THRESH_BINARY)[1] # white over black
cv2.imshow("references", ref)

# find contours in the OCR-A image and sort them from left-to-right
refCnts = cv2.findContours(ref.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
refCnts = imutils.grab_contours(refCnts)
refCnts = refCnts[:-1] # remove border in the copy-past image U-.-
...
for (gX, gY, gW, gH) in locs:
    ...
    print("found digit contours:", len(digitCnts))
